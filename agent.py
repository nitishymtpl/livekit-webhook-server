import os import json import logging import httpx # Or any other HTTP client library from datetime import datetime

from dotenv import load_dotenv

from livekit import agents, rtc # rtc is needed for JobContext if not already imported via agents from livekit.agents import AgentSession, Agent, RoomInputOptions from livekit.plugins import ( openai, noise_cancellation, ) from livekit import api # For Egress and LiveKitAPI client

load_dotenv()

--- Configuration (ideally from environment variables or a config file) ---
LIVEKIT_API_URL = os.getenv("LIVEKIT_URL", "http://localhost:7880") # Your LiveKit server URL LIVEKIT_API_KEY = os.getenv("LIVEKIT_API_KEY") LIVEKIT_API_SECRET = os.getenv("LIVEKIT_API_SECRET")

Azure Blob Storage Configuration
AZURE_STORAGE_ACCOUNT_NAME = os.getenv("AZURE_STORAGE_ACCOUNT_NAME") AZURE_STORAGE_CONTAINER_NAME = os.getenv("AZURE_STORAGE_CONTAINER_NAME") AZURE_STORAGE_ACCOUNT_KEY = os.getenv("AZURE_STORAGE_ACCOUNT_KEY") AZURE_STORAGE_CONNECTION_STRING = os.getenv("AZURE_STORAGE_CONNECTION_STRING") # Added, though account key might be more direct for Egress

URL for your FastAPI analytics backend (where we'll send the data)
ANALYTICS_BACKEND_URL = os.getenv("ANALYTICS_BACKEND_URL", "http://localhost:8000/api/calls/data") ANALYTICS_INTERNAL_API_KEY = os.getenv("ANALYTICS_INTERNAL_API_KEY", "your_internal_api_key_for_analytics_backend")
...
async def entrypoint(ctx: agents.JobContext): logging.info(f"Agent started for room: {ctx.room.name} (SID: {ctx.room.sid})")
...
if name == "main": agents.cli.run_app(agents.WorkerOptions(entrypoint_fnc=entrypoint)) modify this code.
